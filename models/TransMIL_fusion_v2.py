...
class TransMIL_fusion_v2(nn.Module):
    def __init__(self, n_classes=4, input_dim=1024):
        super(TransMIL_fusion_v2, self).__init__()
        ...
        # self.attention_net =s Attn_Net_Gated(L = 512, D = 512, dropout = dropout, n_classes = 1)
    ...
    ##
        ...
        cls_token = torch.concat((cls1, cls2), dim=0)   # torch.Size([2, 512])
        h = cls_token
        A = self.attention(cls_token)
...
if __name__ == "__main__":
    ...
    model = TransMIL_fusion_v2(n_classes=4, input_dim=1024)
